...
router.get("/", (req, res) => {
  ...
  Gallery.findAll({
    ...
    .then((dbGalleryData) => {
      ...
      const galleries = dbGalleryData.map((gallery) => gallery.get({ plain: true })); // serialize all the galleries
      console.log(galleries);
      res.render("home", { galleries, loggedIn: req.session.loggedIn });
    })
    .catch((err) => {
      console.log(err);
...
router.get("/dashboard", (req, res) => {
  ...
  Gallery.findAll({
    ...
    .then((dbGalleryData) => {
      ...
      const galleries = dbGalleryData.map((gallery) => gallery.get({ plain: true })); // serialize all the galleries
      console.log(galleries);
      res.render("dashboard", { galleries, loggedIn: req.session.loggedIn });
    })
    .catch((err) => {
      console.log(err);
      res.status(500).json(err);
